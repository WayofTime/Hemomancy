...
public class TEPlasma extends TileEntity implements IUpdatePlayerListBox
{
	...
	public void spread()
	{
		...
		if(!directionList.isEmpty())
		{
			...
			TEPlasma.createPlasmaBlock(worldObj, getPos().add(facing.getFrontOffsetX(), facing.getFrontOffsetY(), facing.getFrontOffsetZ()), (int) (originalDuration * decayFactor), sizeLimit - (worldObj.rand.nextFloat() < this.dropChance ? 1 : 0));
		}
	}
	
	public static void createPlasmaBlock(World world, BlockPos pos, int duration, int sizeLimit)
	{
		world.setBlockState(pos, ModBlocks.blockPlasma.getDefaultState());
		TEPlasma plasmaBlock = (TEPlasma)world.getTileEntity(pos);
